...
void insert_document(mongoc_collection_t *collection, std::string machine_id,string sensor_id, std::string timestamp_str,std::string alarme, int value) {
    bson_error_t error;
    bson_oid_t oid;
    bson_t *doc;
    ...
    BSON_APPEND_UTF8(doc, "alarme",alarme.c_str());
    ...
    public:
       ...
       if (strncmp(sensor_id.c_str(), "cpu", 3) == 0 || strncmp(sensor_id.c_str(), "mem", 3) == 0 || strncmp(sensor_id.c_str(), "disk", 3) == 0) {
            ...
            if (lastTwoDigits > 10) {
                ...
                insert_document(collection, machine_id, sensor_id.c_str(), timestamp,alarme,value);
                ...
                if(strncmp(sensor_id.c_str(), "mem", 3) == 0 ){
                    if(value>93){
                        ...
                        // Get collection and persist the document.
                         // Get collection and persist the document.
                        mongoc_collection_t *collection = mongoc_database_get_collection(db, "Alarms");
                        insert_document(collection, machine_id, sensor_id.c_str(), timestamp,mem_err,value);
                        mongoc_collection_destroy(collection);
                    ...
                    }else if( strncmp(sensor_id.c_str(), "disk", 3) == 0){
                         if(value>18){
                        ...
                        if(value>70){
                            string mem_err = "cpu está > 60% utilizado";
                            std::clog <<  mem_err << std::endl;
                            // Get collection and persist the document.
                             // Get collection and persist the document.
                            mongoc_collection_t *collection = mongoc_database_get_collection(db, "Alarms");
